...
def check_device_connection(conn_strings: dict) -> dict:
    ...
    try:
        ...
        err = response_custom(
            status.HTTP_404_NOT_FOUND, json.dumps({"error": "Device offline"})
        )
        return err
...
def getSomethingConfig(conn_strings: dict, path: str):
    ...
    target_url = "https://%s:%s/restconf/data/Cisco-IOS-XE-native:native%s" % (
        conn_strings["ipaddr"],
        conn_strings["port"],
        path,
    )
...
def getLoginBanner(conn_strings: dict) -> dict:
    ...
    if len(response.text) > 0:
        try:
            response_body = json.loads(response.text)["Cisco-IOS-XE-native:login"][
                "banner"
            ]
        except:
            response_body = json.loads(response.text)
...
def setLoginBanner(conn_strings: dict, req_to_change: dict) -> dict:
    if len(req_to_change["login_banner"]) > 0:
        body = json.dumps(
            {"login": {"banner": req_to_change["login_banner"]}}, indent=2
        )
...
def getMotdBanner(conn_strings: dict) -> dict:
    ...
    if len(response.text) > 0:
        try:
            response_body = json.loads(response.text)["Cisco-IOS-XE-native:motd"][
                "banner"
            ]
        except:
            response_body = json.loads(response.text)
...
def getPassEncryption(conn_strings: dict) -> dict:
    ...
    if (
        "password-encryption"
        in json.loads(response.text)["Cisco-IOS-XE-native:service"]
    ):
        response_body = "true"
    else:
        response_body = "false"
...
def setPassEncryption(conn_strings, req_to_change: dict):
    if req_to_change["password-encryption"] == "true":
        body = json.dumps(
            {"Cisco-IOS-XE-native:service": {"password-encryption": [None]}}
        )
        response = patchSomethingConfig(conn_strings, "/service", body)
    elif (
        req_to_change["password-encryption"] == "false"
        and getPassEncryption(conn_strings)["body"] == "true"
    ):
...
def getInterfaceList(conn_strings: dict) -> dict:
    ...
    target_url = (
        "https://%s:%s/restconf/data/ietf-interfaces:interfaces/interface?fields=name"
        % (conn_strings["ipaddr"], conn_strings["port"])
    )
    ...
    if len(response.text) > 0:
        try:
            response_body_interface_list = json.loads(response.text)[
...
def getInterfaceDetail(conn_strings: dict, req_to_show: dict) -> dict:
    ...
    target_url = (
        "https://%s:%s/restconf/data/ietf-interfaces:interfaces/interface=%s"
        % (conn_strings["ipaddr"], conn_strings["port"], req_to_show["name"])
    )
        ...
        try:
            ...
            if "address" in interface_data["ietf-ip:ipv4"]:
                response_body = {
                    ...
                    "ip": interface_data["ietf-ip:ipv4"]["address"][0]["ip"],
                    "netmask": interface_data["ietf-ip:ipv4"]["address"][0]["netmask"],
                }
            else:
                response_body = {
                    "name": interface_data["name"],
...
def setInterfaceDetail(conn_strings: dict, req_to_change: dict) -> dict:
    ...
    target_url = (
        "https://%s:%s/restconf/data/ietf-interfaces:interfaces/interface=%s"
        % (conn_strings["ipaddr"], conn_strings["port"], req_to_change["name"])
    )

    try:
        try:
            int_type = getInterfaceDetail(
                conn_strings, {"name": req_to_change["name"]}
            )["body"]["type"]
        except:
            int_type = None
        body = json.dumps(
            {
                "ietf-interfaces:interface": {
                    "name": req_to_change["name"],
                    "type": int_type,
                    "enabled": req_to_change["enabled"],
                    "ietf-ip:ipv4": {
                        "address": [
                            {
                                "ip": req_to_change["ip"],
                                "netmask": req_to_change["netmask"],
                            }
                        ]
                    },
                    "ietf-ip:ipv6": {},
...
def getStaticRoute(conn_strings: dict) -> dict:
    response = getSomethingConfig(
        conn_strings, "/ip/route/ip-route-interface-forwarding-list"
    )
    if len(response.text) > 0:
        try:
            response_body = json.loads(response.text)[
    ...
    body = json.dumps(
        {
            "Cisco-IOS-XE-native:route": {
                "ip-route-interface-forwarding-list": req_to_change
            }
        },
        indent=2,
    )
...
def getOspfProcesses(conn_strings: dict) -> list:
    response = getSomethingConfig(
        conn_strings, "/router/Cisco-IOS-XE-ospf:router-ospf/ospf/process-id?fields=id"
    )
    if len(response.text) > 0:
        try:
...
def createOspfProcess(conn_strings: dict, req_to_create: dict):
    body = json.dumps(
        {"Cisco-IOS-XE-ospf:process-id": {"id": req_to_create["id"]}}, indent=2
    )
    response = postSomethingConfig(
        conn_strings, "/router/Cisco-IOS-XE-ospf:router-ospf/ospf/", body
    )
    try:
        response_body = json.loads(response.text)
    except:
...
def delOspfProcess(conn_strings: dict, req_to_del: dict) -> list:
    response = delSomethingConfig(
        conn_strings,
        "/router/Cisco-IOS-XE-ospf:router-ospf/ospf/process-id=%s" % (req_to_del["id"]),
    )
    try:
        response_body = json.loads(response.text)
    except:
...
def getOspfProcessDetail(conn_strings: dict, req_to_show: dict) -> dict:
    response = getSomethingConfig(
        conn_strings,
        "/router/Cisco-IOS-XE-ospf:router-ospf/ospf/process-id=%s"
        % (req_to_show["id"]),
    )
    if len(response.text) > 0:
        try:
            ...
            if "redistribute" in response_body:
                response_body["redistribute"] = list(
                    response_body["redistribute"].keys()
                )
            else:
                response_body["redistribute"] = []

            if "passive-interface" in response_body:
                response_body["passive-interface"] = response_body["passive-interface"][
                    "interface"
                ]
            else:
...
def setOspfProcessDetail(conn_strings: dict, req_to_change: dict) -> dict:
    body = {
        "Cisco-IOS-XE-ospf:process-id": {
            "id": req_to_change["id"],
            "network": req_to_change["network"],
            "passive-interface": {"interface": req_to_change["passive-interface"]},
        }
    }
...
def getAclList(conn_strings: dict) -> dict:
    response = getSomethingConfig(
        conn_strings=conn_strings, path="/ip/access-list/standard?fields=name"
    )
    if len(response.text) > 0:
        try:
            response_body = []
...
def createAcl(conn_strings: dict, req_to_create: dict) -> dict:
    body = json.dumps(
        {"Cisco-IOS-XE-acl:standard": [{"name": req_to_create["name"]}]}, indent=2
    )
...
def getAclDetail(conn_strings: dict, req_to_show: dict) -> dict:
    response = getSomethingConfig(
        ...
        path="/ip/access-list/standard=%s" % (req_to_show["name"]),
    )
    if len(response.text) > 0:
        ...
        try:
            ...
            if "access-list-seq-rule" in acl_data:
                ...
                for acl_sequence in acl_data["access-list-seq-rule"]:
                    ...
                    prefix = (
                        acl_sequence[action]["std-ace"]["ipv4-prefix"]
                        if "ipv4-prefix" in acl_sequence[action]["std-ace"]
                        else "any"
                    )
                    mask = (
                        acl_sequence[action]["std-ace"]["mask"]
                        if "mask" in acl_sequence[action]["std-ace"]
                        else None
                    )
                    sequences.append(
                        {
        ...
        interface_config = json.loads(
            getSomethingConfig(conn_strings, "/interface").text
        )
        if not "error" in interface_config:
            for config_interface_type in interface_config[
                "Cisco-IOS-XE-native:interface"
            ]:
                for idx, config_interface_setting in enumerate(
                    interface_config["Cisco-IOS-XE-native:interface"][
                        config_interface_type
                    ]
                ):
                    try:
                        if (
                            config_interface_setting["ip"]["access-group"]["out"][
                                "acl"
                            ]["acl-name"]
                            == req_to_show["name"]
                        ):
                            response_body["apply_to_interface"][
                                config_interface_type + str(int(idx) + 1)
                            ] = []
                            response_body["apply_to_interface"][
                                config_interface_type + str(int(idx) + 1)
                            ].append("out")
                    ...
                    try:
                        if (
                            config_interface_setting["ip"]["access-group"]["in"]["acl"][
                                "acl-name"
                            ]
                            == req_to_show["name"]
                        ):
...
def setAclDetail(conn_strings: dict, req_to_change: dict) -> dict:
        ...
        for rule in req_to_change["rules"]:
            if rule["wildcard"]:
                ...
                rules.append(
                    {
                        "sequence": rule["sequence"],
                        rule["action"]: {"std-ace": {"ipv4-prefix": rule["prefix"]}},
                    }
                )
        body = json.dumps(
            {
                "Cisco-IOS-XE-acl:standard": {
                    "name": req_to_change["name"],
                    "access-list-seq-rule": rules,
                }
            }
        )
        response_acledit = putSomethingConfig(
            conn_strings, "/ip/access-list/standard=%s" % (req_to_change["name"]), body
        )
                ...
                for idx, config_interface_setting in enumerate(
                    ...
                    try:
                        if (
                            ...
                            del interface_config["Cisco-IOS-XE-native:interface"][
                                config_interface_type
                            ][idx]["ip"]["access-group"]["out"]["acl"]
                    except:
                        pass
                    ...
                    for interface_name in apply_to_interface_config:
                        ...
                        for direction in apply_to_interface_config[interface_name]:
                            try:
                                interface_config["Cisco-IOS-XE-native:interface"][
                                    interface_type
                                ][int(interface_number) - 1]["ip"]["access-group"][
                                    direction
                                ] = {
                                    "acl": {
                                        "acl-name": req_to_change["name"],
                                        direction: [None],
                                    }
                                }
                            except KeyError:
    ...
    if response_acledit.status_code == 204 and response_aclapply.status_code == 204:
        ...
        return {
            ...
            "body": {
                "acledit": response_body_acledit,
                "aclapply": response_body_aclapply,
            },
        }
...
def delAcl(conn_strings: dict, req_to_del: dict) -> list:
    # delete acl entry
    response_acldelete = delSomethingConfig(
        conn_strings, "/ip/access-list/standard=%s" % (req_to_del["name"])
    )
    ...
    if interface_config_res.status_code == 204:
        ...
        for config_interface_type in interface_config["Cisco-IOS-XE-native:interface"]:
            for idx, config_interface_setting in enumerate(
                interface_config["Cisco-IOS-XE-native:interface"][config_interface_type]
            ):
                try:
                    if (
                        config_interface_setting["ip"]["access-group"]["out"]["acl"][
                            "acl-name"
                        ]
                        == req_to_del["name"]
                    ):
    ...
    if (
        response_acldelete.status_code == 204
        and response_aclapply_delete.status_code == 204
    ):
        return {"code": status.HTTP_204_NO_CONTENT, "body": {}}
    elif (
        response_acldelete.status_code == 404
        or response_aclapply_delete.status_code == 404
    ):
        return {"code": status.HTTP_404_NOT_FOUND, "body": {}}
    else:
        try:
        ...
        return {
            "code": status.HTTP_500_INTERNAL_SERVER_ERROR,
            "body": {
                "acldelete": response_body_acldelete,
                "aclapply delete": response_body_aclapply_delete,
            },
        }
...
def check_device_detector_config(conn_strings: dict, req_to_check: dict) -> dict:
    ...
    def parallel_record():
        ...
        record_config_current = getSomethingConfig(
            conn_strings, "/flow/record=RETIA_RECORD"
        )

    def paralell_exporter():
        global exporter_config_current
        exporter_config_current = getSomethingConfig(
            conn_strings, "/flow/exporter=RETIA_EXPORTER"
        )

    def parallel_monitor():
        global monitor_config_current
        monitor_config_current = getSomethingConfig(
            conn_strings, "/flow/monitor=RETIA_MONITOR"
        )
    ...
    if (
        record_config_current.status_code == 404
        or exporter_config_current == 404
        or monitor_config_current == 404
        or interfaces_current == 404
    ):
        return {
            "synced": None,
            "status": {"error": "Device not configured for netflow"},
        }
    else:
        if record_config_current.status_code == 200:
            record_config_optimal = {
                "Cisco-IOS-XE-flow:record": {
                    ...
                    "collect": {
                        ...
                        "interface": {"output": {}},
                        "routing": {"destination": {"as": {}}, "source": {"as": {}}},
                        "timestamp": {"sys-uptime": {"first": [None], "last": [None]}},
                    },
                    "description": "USED FOR RETIA, DO NOT CHANGE",
                    "match": {
                        "interface": {"input": {}},
                        "ipv4": {
        ...
        if exporter_config_current.status_code == 200:
            ...
            for char in req_to_check["device_interface_to_filebeat"]:
                if char.isalpha():
                    ...
                    interface_number += char
            exporter_config_optimal = {
                "Cisco-IOS-XE-flow:exporter": {
                    "name": "RETIA_EXPORTER",
                    "description": "USED FOR RETIA, DO NOT CHANGE",
                    "destination": {"ipdest": {"ip": req_to_check["filebeat_host"]}},
                    "option": {
                        "application-attributes": {"timeout": 300},
                        "application-table": {"timeout": 60},
                    },
                    "source": {str(interface_type): str(interface_number)},
                    "template": {"data": {"timeout": 60}},
                    "transport": {"udp": req_to_check["filebeat_port"]},
                }
            }
        ...
        if monitor_config_current.status_code == 200:
            monitor_config_optimal = {
                "Cisco-IOS-XE-flow:monitor": {
                    "name": "RETIA_MONITOR",
                    "cache": {"timeout": {"active": 60}},
                    "description": "USED FOR RETIA, DO NOT CHANGE",
                    "exporter": [{"name": "RETIA_EXPORTER"}],
                    "record": {"type": "RETIA_RECORD"},
                }
            }
        ...
        for char in req_to_check["device_interface_to_server"]:
            if char.isalpha():
                ...
                interface_number += char
        interfaces_current_data = json.loads(interfaces_current.text)[
            "Cisco-IOS-XE-native:interface"
        ]
        ...
        for interface_types in interfaces_current_data:
            for interface in interfaces_current_data[interface_types]:
                try:
                    flow_interface_setting = interface["ip"]["Cisco-IOS-XE-flow:flow"][
                        "monitor"
                    ][0]
                    if flow_interface_setting == {
                        "name": "RETIA_MONITOR",
                        "output": [None],
                    }:
                        if (
                            interface_types == interface_type
                            and interface["name"] == interface_number
                        ):
                            flow_monitor_applied_to_correct_interface *= True
                        else:
        ...
        if (
            record_config_current_status == "OK"
            and exporter_config_current_status == "OK"
            and monitor_config_current_status == "OK"
        ):
            synced = True
        else:
            synced = False
        return {
            "synced": synced,
            "status": {
                "flow_record": record_config_current_status,
                "flow_exporter": exporter_config_current_status,
                "flow_monitor": monitor_config_current_status,
                "applied_to_interface": flow_monitor_applied_to_correct_interface,
            },
        }
...
def sync_device_detector_config(conn_strings: dict, req_to_change: dict) -> dict:
    ...
    def parallel_flow():
                    ...
                    "Cisco-IOS-XE-flow:record": [
                        {
                            ...
                            "collect": {
                                ...
                                "timestamp": {
                                    "sys-uptime": {"first": [None], "last": [None]}
                                },
                            },
                            "description": "USED FOR RETIA, DO NOT CHANGE",
                            "match": {
                    ...
                    "Cisco-IOS-XE-flow:exporter": [
                        {
                            ...
                            "description": "USED FOR RETIA, DO NOT CHANGE",
                            "destination": {
                                "ipdest": {"ip": req_to_change["filebeat_host"]}
                            },
                            "option": {
                                "application-attributes": {"timeout": 300},
                                "application-table": {"timeout": 60},
                            ...
                            "source": {str(interface_type): str(interface_number)},
                            "template": {"data": {"timeout": 60}},
                            "transport": {"udp": req_to_change["filebeat_port"]},
                        }
                    ],
                    "Cisco-IOS-XE-flow:monitor": [
                        {
        ...
        response_flow_config = putSomethingConfig(
            conn_strings, "/Cisco-IOS-XE-native:flow", body
        )
        ...
        if interfaces_current.status_code == 200:
            ...
            for interface_types in interfaces_current_data:
                for interface in interfaces_current_data[interface_types]:
                    delSomethingConfig(
                        conn_strings=conn_strings,
                        path="/interface/%s=%s/ip/flow"
                        % (interface_types, interface["name"]),
                    )
        else:
        ...
        body = json.dumps(
            {
                "Cisco-IOS-XE-flow:flow": {
                    "monitor": [{"name": "RETIA_MONITOR", "output": [None]}]
                }
            }
        )
        response_interface_flow_config = putSomethingConfig(
            conn_strings,
            "/interface/%s=%s/ip/flow" % (interface_type, interface_number),
            body,
        )
    ...
    if (
        response_flow_config.status_code == 204
        and response_interface_flow_config.status_code == 204
    ):
        return {"code": status.HTTP_204_NO_CONTENT, "body": {}}
    else:
        try:
        ...
        try:
            response_body_interface_flow_config = json.loads(
                response_interface_flow_config.text
            )
        except:
        ...
        return {
            "code": status.HTTP_500_INTERNAL_SERVER_ERROR,
            "body": {
                "flow_config": response_body_flow_config,
                "flow_interface_config": response_body_interface_flow_config,
            },
        }
...
def del_device_detector_config(conn_strings: dict) -> dict:
    ...
    if interfaces_current.status_code == 200:
        ...
        for interface_types in interfaces_current_data:
            for interface in interfaces_current_data[interface_types]:
                interface_flow_del_response = delSomethingConfig(
                    conn_strings=conn_strings,
                    path="/interface/%s=%s/ip/flow"
                    % (interface_types, interface["name"]),
                )
    ...
    if (
        interface_flow_del_response.status_code == 204
        and flow_del_response.status_code == 204
    ):
        return {"code": status.HTTP_204_NO_CONTENT}
    else:
        try:
            response_body_interface_flow_del = json.loads(
                interface_flow_del_response.text
            )
        except:
        ...
        return {
            "code": status.HTTP_500_INTERNAL_SERVER_ERROR,
            "body": {
                "flow_config": response_body_interface_flow_del,
                "flow_interface_config": response_body_flow_del,
            },
        }
...
def getMonitorBuildinfo():
    prometheus_buildinfo = json.loads(
        requests.get(url="http://localhost:9090/api/v1/status/buildinfo").text
    )["data"]
    alertmanager_buildinfo = json.loads(
        requests.get(url="http://localhost:9093/api/v1/status").text
    )["data"]["versionInfo"]
    snmpexporter_buildinfo = json.loads(
        requests.get(
            url="http://localhost:9090/api/v1/query?query=snmp_exporter_build_info"
        ).text
    )["data"]["result"][0]["metric"]

    response_body = {
        "prometheus": {
            "version": prometheus_buildinfo["version"],
            "revision": prometheus_buildinfo["revision"],
            "goVersion": prometheus_buildinfo["goVersion"],
        },
        "alertmanager": {
            "version": alertmanager_buildinfo["version"],
            "revision": alertmanager_buildinfo["revision"],
            "goVersion": alertmanager_buildinfo["goVersion"],
        },
        "snmpexporter": {
            "version": snmpexporter_buildinfo["version"],
            "revision": snmpexporter_buildinfo["revision"],
            "goVersion": snmpexporter_buildinfo["goversion"],
        },
    }
...
def getAlertStatus():
    response_body = json.loads(
        requests.get(url="http://localhost:9090/api/v1/rules").text
    )["data"]["groups"]
    return response_body
...
def getSysUpTime(mgmt_ipaddr: str):
    try:
        response_body = json.loads(
            requests.get(
                url="http://localhost:9090/api/v1/query?query=sysUpTime{instance='%s'}/100"
                % (mgmt_ipaddr)
            ).text
...
def getDeviceUpStatus(mgmt_ipaddr: str):
    try:
        response_body = json.loads(
            requests.get(
                url="http://localhost:9090/api/v1/query?query=up{instance='%s'}"
                % (mgmt_ipaddr)
            ).text
        )["data"]["result"][0]["value"][1]
    except:
...
def getIntUpStatus(mgmt_ipaddr: str, int_name: str):
    try:
        response_body = json.loads(
            requests.get(
                url="http://localhost:9090/api/v1/query?query=ifOperStatus{instance='%s',ifDescr='%s'}"
                % (mgmt_ipaddr, int_name)
            ).text
        )["data"]["result"][0]["value"][1]
    except:
        response_body = {}
...
def getInterfaceInThroughput(mgmt_ipaddr: str, int_name: str, start_time, end_time):
    ...
    target_url = (
        "http://localhost:9090/api/v1/query_range?query=irate(ifHCInOctets{instance='%s',ifDescr='%s'}[30s])*8 OR on() vector(0)&start=%s&end=%s&step=%s"
        % (mgmt_ipaddr, int_name, start_time, end_time, step)
    )

    try:
        response_body_temp = json.loads(requests.get(url=target_url).text)["data"][
            "result"
        ]
        if len(response_body_temp) > 1:
            ...
            response_body = sorted(
                response_body_temp_appended, key=lambda sublist: sublist[0]
            )
        else:
            response_body = json.loads(requests.get(url=target_url).text)["data"][
                "result"
            ][0]["values"]
    except:
        response_body = {}
    for idx, metric_item in enumerate(response_body):
        response_body[idx][0] = datetime.fromtimestamp(
            metric_item[0], tz=tzlocal.get_localzone()
...
def getInterfaceOutThroughput(mgmt_ipaddr: str, int_name: str, start_time, end_time):
    ...
    target_url = (
        "http://localhost:9090/api/v1/query_range?query=irate(ifHCOutOctets{instance='%s',ifDescr='%s'}[30s])*8 OR on() vector(0)&start=%s&end=%s&step=%s"
        % (mgmt_ipaddr, int_name, start_time, end_time, step)
    )
    try:
        response_body_temp = json.loads(requests.get(url=target_url).text)["data"][
